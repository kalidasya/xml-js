...
getFixtureFiles('.xml').map(function (file) {
  ...
  writeFileSync(file.replace('.xml','1.json'), xml2json(readFileSync(file, 'utf8'), options), 'utf8');
});

getFixtureFiles('.xml').map(function (file) {
  console.log('converting %s to %s', file, file.replace('.xml','2.json'));
  var options = {compact: false, spaces: 4};
  writeFileSync(file.replace('.xml','2.json'), xml2json(readFileSync(file, 'utf8'), options), 'utf8');
});
