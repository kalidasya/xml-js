import pkg from 'sax';
const { parser: _parser } = pkg;
var expat /*= require('node-expat');*/ = { on: function () { }, parse: function () { } };
import { copyOptions, ensureFlagExists, ensureAlwaysArrayExists, ensureKeyExists, checkFnExists } from './options-helper.js';
...
function validateOptions(userOptions) {
  options = copyOptions(userOptions);
  ...
  ensureFlagExists('addParent', options);
  ensureFlagExists('trim', options);
  ensureFlagExists('nativeType', options);
  ensureFlagExists('nativeTypeAttributes', options);
  ...
  ensureFlagExists('trackPosition', options);
  ensureAlwaysArrayExists(options);
...
const getPositionInfo = ({ column, line, startTagPosition }) => ({ column, line, startTagPosition });
...
function addField(type, value) {
  ...
  if (options.compact) {
    if (
      ...
      (Array.isArray(options.alwaysArray) ? options.alwaysArray.indexOf(options[type + 'Key']) !== -1 : options.alwaysArray)
    ) {
      currentElement[options[type + 'Key']] = [];
    }
    if (currentElement[options[type + 'Key']] && !Array.isArray(currentElement[options[type + 'Key']])) {
    ...
    if (Array.isArray(currentElement[options[type + 'Key']])) {
...
function onStartElement(name, attributes) {
  ...
  if (options.compact) {
    ...
    if (
      ...
      (Array.isArray(options.alwaysArray) ? options.alwaysArray.indexOf(name) !== -1 : options.alwaysArray)
    ) {
      currentElement[name] = [];
    }
  ...
  if (options.trackPosition && !options.compact) {
    currentElement.position = {start : getPositionInfo(this), end : null};
  }
}
...
function onEndElement(name) {
  ...
  // TODO add end position to current or create a new callback
  if (!options.addParent) {
    delete currentElement[options.parentKey];
  }
  if (options.trackPosition && !options.compact) {
    currentElement.position.end = getPositionInfo(this)
  }
...
export default function (xml, userOptions) {
  ...
  var parser = pureJsParser ? _parser(true, {}) : parser = new expat.Parser('UTF-8');
  var result = {};
